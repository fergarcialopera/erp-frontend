      ...
      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] sm:text-xs font-medium text-muted-foreground">
        ...
        {data && !loading && (
          <span className="font-normal tabular-nums">
            · stock total <span className="text-foreground">{data.quantity_total}</span>
          </span>
        )}
      </div>

      {loading && <p className="text-xs text-muted-foreground">Cargando ubicaciones…</p>}

      {!loading && locations.length === 0 && (
        <p className="text-xs text-muted-foreground">
          Este producto no tiene stock en ambientes ni zonas.
        </p>
      )}
            ...
            return (
              ...
              >
                <StockLocationDisplay
                  {...resolveStockLocationLabels(location.ambiente, location.zone)}
